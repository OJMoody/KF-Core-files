...
defaultproperties
{
	...
	// Red Orchestra defaults

	BeginningIdleAnim = idle_open
 	bAllowViewChange=false // Don't allow behindview
 	bMultiPosition=true

	VehHitpoints(0)=(PointRadius=9.0,PointHeight=0.0,PointScale=1.0,PointBone=driver_player,PointOffset=(X=0.0,Y=0.0,Z=0.0),bPenetrationPoint=true,DamageMultiplier=0.0,HitPointType=HP_Driver)
	VehHitpoints(1)=(PointRadius=25.0,PointHeight=0.0,PointScale=1.0,PointBone=body,PointOffset=(X=0.0,Y=0.0,Z=0.0),bPenetrationPoint=false,DamageMultiplier=0.0,HitPointType=HP_Engine)

	EngineHealth=100

	AmbientSoundScaling=1.25
	CurrentCapArea=255
	IdleTimeBeforeReset = 10.0
	bEnterringUnlocks=false
	TimeTilDissapear=60
	// Debugging mesh swapping
	bDontUsePositionMesh=false
	bLimitYaw = true
	bDisableThrottle=true
	bMustBeTankCommander=false;

	DriverPositions(0)=(ViewFOV=85);
	DriverPositions(1)=(ViewFOV=85);
// Hud
	// Speed Debugging
	...
	MPHMeterScale=70.0
	MPHMeterSizeY=0.05
	bDrawSpeedDebug=false
	// Make the vehicle hud be drawn
	bSpecialHUD=true

	VehicleSpikeTime=3.0


// Onslaught variables we want to modify or use
	 DriverDamageMult=0.800000
	RanOverDamageType=class'DamRanOver'
	CrushedDamageType=class'Crushed'
	 bReplicateAnimations=True
	 SoundVolume=255 // All the way up
	 bPCRelativeFPRotation=True
	 bDrawDriverInTP=True
	 bDrawMeshInFP=True
	 DriveAnim=VPanzer4_driver_idle_close // Generic, just in case we don't specify one

// Onslaught variables we want to override
	StolenSound = none
	LockedOnSound = none
	StolenAnnouncement = ''
	SpawnOverlay(0) = none
	SpawnOverlay(1) = none
	FenderBenderSpeed = 10000000 		// set fenderbender speed to something high so it never goes off
	bTeamLocked = true
	bAllowAirControl = false			// don't allow air control
	NoEntryTexture=none
	TeamBeaconTexture=none
